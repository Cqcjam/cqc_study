...
public class IdKit {
	...
	private final static Siri siri = new Siri(0);
			...
			while (true) {
				...
				if (lastTimestamp == now) {
					...
					SimpleDateFormat sdf = new SimpleDateFormat(
							"yyyyMMddHHmmss");
					return Long.parseLong(String.format("%s%d%03d",
							sdf.format(new Date(now)), workerId, now % 1000));
				}
			}
	...
	private static class Snowflake {
		...
		public Snowflake(long datacenterId) {
			workerId=Long.parseLong(PropertiesKit.get("snowflake.workerId")) ;
			// sanity check for workerId
